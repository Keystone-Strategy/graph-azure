...
import {
  ...
  } from '../../active-directory/client';

import * as  _ from 'lodash'
import { ATTACHMENT_ENTITY_TYPE, CONVERSATION_ENTITY_TYPE, DOMAIN_ENTITY_TYPE, EMAIL_ADDRESS_ENTITY_TYPE, MESSAGE_ENTITY_TYPE } from '../constants';
...
export default async function fetchMessages(
  ...
  await graphClient.iterateUserMessages({ 
    ...
    async (message:any) => {
      const alreadyExistingMessageEntity = await jobState.findEntity(message.id);
      ...
      const conversationEntity = createConversationEntity(message.conversationId)
      ...
      const {
        ...
        emailAddressBelongsToDomainRelationships: emailAddressBelongsToDomainRelationshipsFromRecipients
      ...
      const {
        ...
        emailAddressBelongsToDomainRelationships: emailAddressBelongsToDomainRelationshipsFromCCs
      } = processCCs(message, messageEntity)
      
      const entities:any= []
      entities.push(messageEntity)
      entities.push(conversationEntity)
      if (fromEmailAddressEntity !== null) entities.push(fromEmailAddressEntity)
      ...
      if (messageSentFromEmailAddressRelationship !== null) relationships.push(messageSentFromEmailAddressRelationship)
      if (emailAddressBelongsToDomainRelationship !== null) relationships.push(emailAddressBelongsToDomainRelationship)
      relationships.push(...messageSentToEmailAddressRelationships)
      relationships.push(...emailAddressBelongsToDomainRelationshipsFromRecipients)
      ...
      const attachments = await graphClient.listAttachments(instance.config.exchangeUserId, message.id)
...
const getDomainFromEmailAddress = address => {
  ...
  const domain = `${splitDot[splitDot.length - 2]}.${splitDot[splitDot.length - 1]}`
  return domain.toLowerCase()
}

const processSender = (message, messageEntity) => {
  const fromEmailAddressAddress = _.get(message, 'from.emailAddress.address', null)
  ...
  const fromEmailAddressName = _.get(message, 'from.emailAddress.name', 'NOT DEFINED')
  ...
  const fromDomainEntity = fromDomain !== null ? createDomainEntity(fromDomain) : null

  const emailAddressBelongsToDomainRelationship = fromDomainEntity !== null ?  createDirectRelationship({
...
const processRecipients = (message, messageEntity) => {
  ...
  for (const toEmailRecipient of message.toRecipients) {
    const toEmailAddressAddress = _.get(toEmailRecipient, 'emailAddress.address', null)
    
    if (!toEmailAddressAddress) continue

    const toEmailAddressName = _.get(toEmailRecipient, 'emailAddress.name', 'NOT DEFINED')
    ...
    const toDomainEntity = toDomain !== null ? createDomainEntity(toDomain) : null

    const emailAddressBelongsToDomainRelationship = toDomainEntity !== null ? createDirectRelationship({
    ...
    messageSentToEmailAddressRelationships.push(messageSentToEmailAddressRelationship)    
    if (emailAddressBelongsToDomainRelationship !== null) emailAddressBelongsToDomainRelationships.push(emailAddressBelongsToDomainRelationship)
  }
...
const processCCs = (message, messageEntity) => {
  ...
  for (const ccEmailRecipient of message.ccRecipients) {
    const ccEmailAddressAddress = _.get(ccEmailRecipient, 'emailAddress.address', null)
    
    if (!ccEmailAddressAddress) continue

    const ccEmailAddressName = _.get(ccEmailRecipient, 'emailAddress.name', 'NOT DEFINED')
    ...
    const ccDomainEntity = ccDomain !== null ? createDomainEntity(ccDomain) : null

    const emailAddressBelongsToDomainRelationship = ccDomainEntity !== null ? createDirectRelationship({
    ...
    messageSentCCEmailAddressRelationships.push(messageSentCCEmailAddressRelationship)    
    if (emailAddressBelongsToDomainRelationship !== null) emailAddressBelongsToDomainRelationships.push(emailAddressBelongsToDomainRelationship)
  }
...
const storeRelationships = async (relationships, jobState: JobState) => {
  const uniqueRelationships = _.uniqBy(relationships, (r:Relationship) => r._key)
